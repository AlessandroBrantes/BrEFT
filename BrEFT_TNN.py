...
def train_neural_network(symbol, start_date, end_date, lookback, learning_rate, batch_size, l2_lambda, hidden_layers):
    try:
        ...
        if os.path.exists(model_file):
            ...
            # Compile the model
            model.compile(loss="mean_squared_error", optimizer=Adam(lr=learning_rate), metrics=['mean_absolute_error', 'r2'])

        # Configure early stopping
        early_stopping = EarlyStopping(monitor='val_loss', patience=10, verbose=1, mode='min', restore_best_weights=True)

        # Adicione o histórico de treinamento como um callback
        history = History()
        
        # Train the model with early stopping and history callback
        model.fit(X_train, y_train, epochs=100, batch_size=batch_size, verbose=1, validation_data=(X_val, y_val), callbacks=[early_stopping, history])

        # Imprima as métricas 'mae' e 'r2' após o treinamento
        ...
        print(f"Coeficiente de determinação (R2): {r2_train:.4f}")

        print(f"\nMétricas de validação:")
        print(f"Mean Absolute Error (MAE): {history.history['val_mean_absolute_error'][-1]:.4f}")
        print(f"Coeficiente de determinação (R2): {history.history['val_r2'][-1]:.4f}")
